...
import { Box, Bird, Cat, Rabbit, Users, Code2, Wand2, Hammer, FolderOpen, ChevronUp, FileJson, History, Play, Pause, Info, Wrench, Loader2, Video, Sun, Moon } from 'lucide-react';
...
interface UIOverlayProps {
  ...
  onChangeTimeMode: (mode: 'day' | 'night') => void;
}
    ...
    <div className="absolute top-0 left-0 w-full h-full pointer-events-none select-none">
      ...
      <div className="absolute top-4 left-4 right-4 flex justify-between items-start">
        ...
        <div className="pointer-events-auto flex flex-col gap-2">
            ...
            <div className="w-[180px] p-3 bg-white/95 backdrop-blur-md shadow-lg rounded-2xl border border-slate-200/80 flex flex-col gap-2 mt-2 transition-all">
                <div className="flex items-center justify-between">
                    <span className="text-[10px] uppercase tracking-wider font-extrabold text-slate-400">Environment</span>
                    {/* Day / Night Toggle switch */}
                    <div className="flex bg-slate-100/80 p-0.5 rounded-lg border border-slate-250/50">
                        <button
                            onClick={() => onChangeTimeMode('day')}
                            className={`p-1 rounded-md transition-all ${timeMode === 'day' ? 'bg-white text-yellow-500 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                            title="Day Mode"
                            id="ui_day_mode_toggle"
                        >
                            <Sun size={13} fill={timeMode === 'day' ? "currentColor" : "none"} />
                        </button>
                        <button
                            onClick={() => onChangeTimeMode('night')}
                            className={`p-1 rounded-md transition-all ${timeMode === 'night' ? 'bg-slate-700 text-indigo-300 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                            title="Night Mode"
                            id="ui_night_mode_toggle"
                        >
                            <Moon size={13} fill={timeMode === 'night' ? "currentColor" : "none"} />
                        </button>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-1">
                    <div className="flex justify-between items-center text-[9px] font-extrabold text-slate-400 uppercase tracking-wider">
                        <span>Intensity</span>
                        <span className="font-mono text-slate-550 font-bold">{(ambientIntensity * 100).toFixed(0)}%</span>
                    </div>
                    <input
                        type="range"
                        min="0"
                        ...
                        className="w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-indigo-500 hover:accent-indigo-600 transition-all"
                        style={{
                            outline: 'none'
                        }}
                        id="ui_ambient_intensity"
                    />
                </div>
            </div>
        </div>

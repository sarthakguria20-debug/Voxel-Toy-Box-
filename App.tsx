...
const App: React.FC = () => {
  ...
  const handleChangeAmbientIntensity = (intensity: number) => {
  ...
  const handleChangeTimeMode = (mode: 'day' | 'night') => {
      setTimeMode(mode);
      if (engineRef.current) {
          engineRef.current.setTimeMode(mode);
      }
  };
  ...
  return (
    <div className="relative w-full h-screen bg-[#f0f2f5] overflow-hidden">
      ...
      <UIOverlay 
        ...
        onOpenAnimator={() => setIsAnimatorOpen(true)}
        ambientIntensity={ambientIntensity}
        timeMode={timeMode}
        onChangeAmbientIntensity={handleChangeAmbientIntensity}
